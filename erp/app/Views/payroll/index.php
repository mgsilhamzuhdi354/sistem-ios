...
<div class="page-header" style="display: flex; justify-content: space-between; align-items: center;">
    <div>
        ...
        <p>Process crew salaries and taxes</p>
    </div>
    <div style="display: flex; gap: 12px;">
        <a href="<?= BASE_URL ?>payroll/export/<?= $period['id'] ?? 0 ?>" class="btn btn-secondary"><i class="fas fa-download"></i> Export CSV</a>
        <form method="POST" action="<?= BASE_URL ?>payroll/process" style="display: inline;">
            <input type="hidden" name="month" value="<?= $month ?>">
            <input type="hidden" name="year" value="<?= $year ?>">
            <button type="submit" class="btn btn-primary"><i class="fas fa-play"></i> Run Payroll</button>
        </form>
    </div>
</div>
...
<div style="display: flex; align-items: center; gap: 16px; margin-bottom: 24px;">
    <a href="?month=<?= $month == 1 ? 12 : $month - 1 ?>&year=<?= $month == 1 ? $year - 1 : $year ?>" class="btn-icon"><i class="fas fa-chevron-left"></i></a>
    ...
    <a href="?month=<?= $month == 12 ? 1 : $month + 1 ?>&year=<?= $month == 12 ? $year + 1 : $year ?>" class="btn-icon"><i class="fas fa-chevron-right"></i></a>
    <span class="badge badge-<?= $period['status'] === 'completed' ? 'success' : ($period['status'] === 'processing' ? 'warning' : 'secondary') ?>" style="margin-left: 16px;">
...
<div class="grid-4" style="margin-bottom: 24px;">
    <div class="stat-card">
        ...
        <div class="stat-info"><h3><?= $period['total_crew'] ?? 0 ?></h3><p>Total Crew</p></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon gold"><i class="fas fa-money-bill-wave"></i></div>
        <div class="stat-info"><h3>$<?= number_format($period['total_gross'] ?? 0, 2) ?></h3><p>Gross Salary</p></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon red"><i class="fas fa-percentage"></i></div>
        <div class="stat-info"><h3>$<?= number_format($period['total_tax'] ?? 0, 2) ?></h3><p>Total Tax (5%)</p></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon green"><i class="fas fa-check-circle"></i></div>
        <div class="stat-info"><h3>$<?= number_format($period['total_net'] ?? 0, 2) ?></h3><p>Net Payable</p></div>
    </div>
</div>
...
<div class="table-card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        ...
        <span style="font-size: 12px; color: var(--text-muted);">* Semua nilai dalam USD dengan 2 desimal untuk akurasi</span>
    </div>
        ...
        <tbody>
            <?php if (empty($items)): ?>
                <tr><td colspan="11" style="text-align: center; color: var(--text-muted); padding: 40px;">
                    <i class="fas fa-calculator" style="font-size: 40px; margin-bottom: 10px; display: block; opacity: 0.5;"></i>
            ...
            <?php else: ?>
                ...
                <tr>
                    ...
                    <td style="text-align: right;">
                        <?php if ($originalCurrency !== 'USD'): ?>
                            ...
                            <br><span style="font-size: 9px; color: var(--warning);">×<?= number_format($exchangeRate, 6) ?></span>
                        <?php else: ?>
                    ...
                    <td style="text-align: right; color: var(--success); font-weight: 700; font-size: 14px;">$<?= number_format($netSalary, 2) ?></td>
                ...
                <tr style="background: rgba(0,0,0,0.2); font-size: 10px; color: var(--text-muted);">
                    ...
                    <td colspan="7" style="padding: 4px 12px;">
                        Gross ($<?= number_format($grossSalary, 2) ?>) - Deductions ($<?= number_format($totalDeductions, 2) ?>) - Tax ($<?= number_format($taxAmount, 2) ?>) = <strong style="color: var(--success);">Net $<?= number_format($netSalary, 2) ?></strong>
                    </td>
                </tr>
                <?php endforeach; ?>
            <?php endif; ?>
