...
<head>
    ...
    <title><?= $title ?? 'Edit Contract' ?> - IndoOcean ERP</title>
...
<body class="bg-gray-50 font-sans antialiased" x-data="editContractForm()">
    ...
    <main class="ml-64 min-h-screen flex flex-col bg-gray-50">
        ...
        <header class="bg-white border-b border-gray-200 px-8 pt-6 sticky top-0 z-40">
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">
                            ...
                            <span class="ml-2"><?= htmlspecialchars($contract['crew_name'] ?? '') ?></span>
                        </p>
                    </div>
                </div>
                ...
                $statusColor = $statusColors[$contract['status'] ?? 'draft'] ?? 'bg-gray-100 text-gray-600';
                ?>
                <span class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-semibold <?= $statusColor ?>">
            ...
            <div class="flex items-center max-w-6xl mx-auto pb-6">
                <template x-for="(step, index) in steps" :key="index">
                    <div class="flex flex-1 items-center gap-3 pb-4 border-b-2 transition-all cursor-pointer"
        ...
        <!-- Form Content -->
        <form action="<?= BASE_URL ?>contracts/update/<?= $contract['id'] ?>" method="POST" novalidate>
        <div class="flex-1 overflow-y-auto custom-scrollbar p-8">
            <div class="max-w-6xl mx-auto space-y-8">
                ...
                <section x-show="currentStep === 1" x-transition class="space-y-6">
                    ...
                    <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                        <div class="flex items-center gap-3 mb-6">
                            <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white flex items-center justify-center">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                                    <?= __('contracts.contract_number') ?> <span class="text-red-500">*</span>
                                </label>
                                <input type="text" name="contract_no" readonly
                                    value="<?= htmlspecialchars($contract['contract_no'] ?? '') ?>"
                                    class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-500 cursor-not-allowed">
                    ...
                    <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                        <div class="flex items-center gap-3 mb-6">
                            <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-teal-600 to-cyan-600 text-white flex items-center justify-center">
                                ...
                                <select name="currency_id" x-model="formData.currencyId" @change="toggleExchangeRate()"
                                    ...
                                    <?php foreach ($currencies as $cur): ?>
                                        <option value="<?= $cur['id'] ?>" data-code="<?= $cur['code'] ?>"
                                            <?= $currentCurrencyId == $cur['id'] ? 'selected' : '' ?>><?= $cur['code'] ?> - <?= $cur['name'] ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div class="p-5 bg-gray-50 rounded-xl border border-gray-100">
                                ...
                                <input type="text" name="basic_salary" @input="formatCurrency($event.target); calculateTotals()"
                                    value="<?= number_format($contract['basic_salary'] ?? 0, 0) ?>"
                            ...
                            <div class="p-5 bg-gray-50 rounded-xl border border-gray-100">
                                ...
                                <input type="text" name="overtime_allowance" @input="formatCurrency($event.target); calculateTotals()"
                                    value="<?= number_format($contract['overtime_allowance'] ?? 0, 0) ?>"
                                    class="w-full bg-transparent border-none p-0 text-2xl font-bold text-gray-900 focus:ring-0 outline-none"
                                    placeholder="0">
                            ...
                            <div class="p-5 bg-gray-50 rounded-xl border border-gray-100">
                                ...
                                <input type="text" name="leave_pay" @input="formatCurrency($event.target); calculateTotals()"
                                    value="<?= number_format($contract['leave_pay'] ?? 0, 0) ?>"
                                    class="w-full bg-transparent border-none p-0 text-2xl font-bold text-gray-900 focus:ring-0 outline-none"
                                    placeholder="0">
                            </div>
                            ...
                            <div class="p-5 bg-gray-50 rounded-xl border border-gray-100">
                                ...
                                <input type="text" name="bonus" @input="formatCurrency($event.target); calculateTotals()"
                                    value="<?= number_format($contract['bonus'] ?? 0, 0) ?>"
                                    class="w-full bg-transparent border-none p-0 text-2xl font-bold text-gray-900 focus:ring-0 outline-none"
                                    placeholder="0">
                            </div>
                            ...
                            <div class="md:col-span-2 p-5 bg-gray-50 rounded-xl border border-gray-100">
                                ...
                                <input type="text" name="other_allowance" @input="formatCurrency($event.target); calculateTotals()"
                                    value="<?= number_format($contract['other_allowance'] ?? 0, 0) ?>"
                                    class="w-full bg-transparent border-none p-0 text-2xl font-bold text-gray-900 focus:ring-0 outline-none"
                                    placeholder="0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="npwp_number" @input="formatNPWP($event.target)" maxlength="20"
                                    value="<?= htmlspecialchars($contract['npwp_number'] ?? '') ?>"
                                    class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-700 transition-all outline-none"
                        ...
                        <div class="space-y-4" x-ref="deductionsContainer">
                            ...
                            <?php if (!empty($deductions)): ?>
                                <?php foreach ($deductions as $i => $ded): ?>
                                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end p-4 bg-gray-50 rounded-xl border border-gray-200">
                                        ...
                                        <div class="md:col-span-3">
                                            ...
                                            <input type="text" name="deduction_desc[]"
                                        ...
                                        <div class="md:col-span-2">
                                            ...
                                            <input type="text" name="deduction_amount[]"
                                                value="<?= number_format($ded['amount'], 0, '', '') ?>"
                ...
                <!-- Step 4: Review -->
                <section x-show="currentStep === 4" x-transition class="space-y-6">
                    <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                        <div class="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                <h2 class="text-lg font-bold text-gray-900">Review & Update</h2>
                                <p class="text-sm text-gray-500">Review all changes before saving</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div class="p-5 bg-gray-50 rounded-xl border border-gray-100">
                                <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">Contract Info</h3>
                                <div class="space-y-3">
                                    <div class="flex justify-between">
                                        <span class="text-sm text-gray-500">Contract No</span>
                                        <span class="text-sm font-semibold text-gray-900"><?= htmlspecialchars($contract['contract_no']) ?></span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-sm text-gray-500">Crew</span>
                                        <span class="text-sm font-semibold text-gray-900" x-text="formData.crewName || '<?= htmlspecialchars($contract['crew_name'] ?? '') ?>'"></span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-sm text-gray-500">Period</span>
                                        <span class="text-sm font-semibold text-gray-900" x-text="formData.signOnDate + ' → ' + formData.signOffDate"></span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-sm text-gray-500">Duration</span>
                                        <span class="text-sm font-semibold text-gray-900" x-text="formData.duration + ' months'"></span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-5 bg-gray-50 rounded-xl border border-gray-100">
                                ...
                                <div class="space-y-3">
                                    <div class="flex justify-between">
                                        ...
                                        <span class="text-sm font-semibold text-gray-900" x-text="formatMoney(totalSalary)"></span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-sm text-gray-500">Client Rate</span>
                                        <span class="text-sm font-semibold text-emerald-700" x-text="formatMoney(clientRate)"></span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-sm text-gray-500">Deductions</span>
                                        <span class="text-sm font-semibold text-orange-600" x-text="formatMoney(totalDeductions)"></span>
                                    </div>
                                    <div class="flex justify-between pt-3 border-t">
                                        <span class="text-sm font-semibold text-gray-700">Est. Profit/Loss</span>
                                        <span class="text-sm font-bold" 
                                            :class="profit >= 0 ? 'text-emerald-700' : 'text-red-600'"
                                            x-text="formatMoney(profit)"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <footer class="bg-white border-t border-gray-200 px-8 py-5 flex items-center justify-between shadow-lg sticky bottom-0 z-40">
            ...
            <div class="flex items-center gap-4">
                ...
                <button type="button" x-show="currentStep === totalSteps" @click="submitUpdate()"
        ...
        function editContractForm() {
            return {
                ...
                formData: {
                    ...
                    crewName: '<?= addslashes($contract['crew_name'] ?? '') ?>',
                    rankId: '<?= $contract['rank_id'] ?? '' ?>',
                ...
                getCurrencySymbol() {
                    ...
                    const symbols = {
                        'USD': '$', 'IDR': 'Rp ', 'EUR': '€', 'SGD': 'S$',
                        'GBP': '£', 'JPY': '¥', 'MYR': 'RM ', 'AUD': 'A$', 'PHP': '₱',
                    };
