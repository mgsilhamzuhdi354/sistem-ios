...
<div class="grid-4" style="gap: 16px; margin-bottom: 24px;">
    <div class="stat-card">
        ...
        <div class="stat-info">
            ...
            <span class="stat-label" data-translate="status_available">Tersedia</span>
        </div>
    </div>
    <div class="stat-card">
        ...
        <div class="stat-info">
            ...
            <span class="stat-label" data-translate="nav_on_board">Sedang Bekerja</span>
        </div>
    </div>
    <div class="stat-card">
        ...
        <div class="stat-info">
            ...
            <span class="stat-label" data-translate="status_on_leave">Cuti</span>
        </div>
    </div>
    <div class="stat-card">
...
<div class="card" style="margin-bottom: 20px;">
    <form method="GET" action="<?= BASE_URL ?>crews"
        ...
        <div style="flex: 1; min-width: 200px;">
            ...
            <input type="text" name="search" class="form-control" data-translate-placeholder="search_crew"
                placeholder="Nama, ID, email, telepon..." value="<?= htmlspecialchars($filters['search'] ?? '') ?>">
        </div>
        <div style="min-width: 150px;">
            ...
            <select name="status" class="form-control">
                ...
                <option value="available" <?= ($filters['status'] ?? '') === 'available' ? 'selected' : '' ?>
                    data-translate="status_available">Tersedia</option>
                <option value="onboard" <?= ($filters['status'] ?? '') === 'onboard' ? 'selected' : '' ?>
                    data-translate="nav_on_board">Sedang Bekerja</option>
                <option value="standby" <?= ($filters['status'] ?? '') === 'standby' ? 'selected' : '' ?>
                    data-translate="nav_stand_by">Menunggu</option>
                <option value="leave" <?= ($filters['status'] ?? '') === 'leave' ? 'selected' : '' ?>
                    data-translate="status_on_leave">Cuti</option>
                <option value="blacklisted" <?= ($filters['status'] ?? '') === 'blacklisted' ? 'selected' : '' ?>
                    data-translate="status_blacklisted">Blacklisted</option>
            </select>
        </div>
        <button type="submit" class="btn btn-primary">
            <i class="fas fa-search"></i> <span data-translate="btn_filter">Filter</span>
        </button>
        <a href="<?= BASE_URL ?>crews" class="btn btn-secondary">
            <i class="fas fa-times"></i> <span data-translate="btn_reset">Reset</span>
        </a>
    </form>
</div>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th data-translate="employee_id">Employee ID</th>
                <th data-translate="contact">Kontak</th>
                <th data-translate="th_rank">Jabatan</th>
                <th data-translate="th_status">Status</th>
                <th style="width: 140px;" data-translate="th_actions">Aksi</th>
            </tr>
        </thead>
        <tbody>
            <?php if (empty($crews)): ?>
                <tr>
                    <td colspan="7" style="text-align: center; padding: 40px; color: var(--text-muted);">
                        <i class="fas fa-users"
                            style="font-size: 48px; margin-bottom: 16px; display: block; opacity: 0.3;"></i>
                        <span data-translate="no_crew_found">Tidak ada crew ditemukan</span>
                    </td>
                </tr>
            <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display: flex; align-items: center; gap: 12px;">
                                ...
                                <?php else: ?>
                                    <img src="https://ui-avatars.com/api/?name=<?= urlencode($crew['full_name']) ?>&background=0A2463&color=fff&size=40"
                                ...
                                <div>
                                    ...
                                    <?php if ($crew['nickname']): ?>
                                        <div style="font-size: 12px; color: var(--text-muted);">
                                            "<?= htmlspecialchars($crew['nickname']) ?>"</div>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </td>
                        <td>
                            <code
                                style="background: rgba(212, 175, 55, 0.1); color: var(--accent-gold); padding: 4px 8px; border-radius: 4px;">
                        ...
                        <td>
                            <?php if ($crew['phone']): ?>
                                <div><i class="fas fa-phone" style="width: 16px; color: var(--text-muted);"></i>
                                    <?= htmlspecialchars($crew['phone']) ?></div>
                            <?php endif; ?>
                            <?php if ($crew['email']): ?>
                                <div style="font-size: 12px; color: var(--text-muted);"><i class="fas fa-envelope"
                                        style="width: 16px;"></i> <?= htmlspecialchars($crew['email']) ?></div>
                            <?php endif; ?>
                        </td>
                        ...
                        <td>
                            <div class="action-buttons" style="display: flex; gap: 4px;">
                                ...
                                <a href="<?= BASE_URL ?>documents/<?= $crew['id'] ?>" class="btn-icon" title="Documents"
