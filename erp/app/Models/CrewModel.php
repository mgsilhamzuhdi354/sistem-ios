...
class CrewModel extends BaseModel
{
    ...
    protected $allowedFields = [
        'employee_id', 'full_name', 'nickname', 'gender', 'birth_date', 'birth_place',
        'nationality', 'religion', 'marital_status', 'email', 'phone', 'whatsapp',
        'address', 'city', 'province', 'postal_code', 'emergency_name', 'emergency_relation',
        'emergency_phone', 'bank_name', 'bank_account', 'bank_holder', 'current_rank_id',
        'years_experience', 'total_sea_time_months', 'photo', 'status', 'notes', 'created_by'
    ];
...
class CrewSkillModel extends BaseModel
{
    ...
    protected $allowedFields = ['crew_id', 'skill_name', 'skill_level', 'certificate_id', 'notes'];
    
    public function getByCrew($crewId)
    {
        $sql = "SELECT * FROM {$this->table} WHERE crew_id = ? ORDER BY skill_name";
        return $this->query($sql, [$crewId], 'i');
    }
    
    public function deleteAllByCrew($crewId)
    {
        $sql = "DELETE FROM {$this->table} WHERE crew_id = ?";
        return $this->execute($sql, [$crewId], 'i');
    }
}
...
class CrewExperienceModel extends BaseModel
{
    ...
    protected $allowedFields = [
        'crew_id', 'vessel_name', 'vessel_type', 'vessel_flag', 'gross_tonnage',
        'engine_type', 'company_name', 'rank_position', 'start_date', 'end_date',
        'reason_leaving', 'reference_contact'
    ];
...
class CrewDocumentModel extends BaseModel
{
    ...
    protected $allowedFields = [
        'crew_id', 'document_type', 'document_name', 'document_number', 'file_path',
        'file_name', 'file_size', 'mime_type', 'issue_date', 'expiry_date',
        'issuing_authority', 'issuing_place', 'status', 'verified_by', 'verified_at',
        'notes', 'uploaded_by'
    ];
...
class DocumentTypeModel extends BaseModel
{
    ...
    protected $allowedFields = [
        'code', 'name', 'name_id', 'description', 'is_mandatory',
        'validity_years', 'reminder_days', 'category', 'sort_order', 'is_active'
    ];
